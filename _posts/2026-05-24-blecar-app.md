---
layout: post
title:  "arduino蓝牙车app控制"
author: "David"
header-style: text
tags: 
    - stem教育
    - Maker
---

## 远程控制（手机端）
使用了app inventor app来控制蓝牙车运动。国内可以使用[WxBit  ](https://www.wxbit.com/)来创建app。国外可以使用[MIT App Inventor](https://ai2.mit.edu/appinventor/)也可以创建app。

app inventor端：
界面：
![](https://images-1303887003.cos.ap-beijing.myqcloud.com//images/appinventor_ble_car2.png)
后台：
![控制](https://images-1303887003.cos.ap-beijing.myqcloud.com//images/appinventor_ble_car.png)

## 语音控制

将加入百度语音识别功能，实现通过语音来控制蓝牙车的运动。comming soon...

小车控制代码见[蓝牙车再探](https://davidit.top/2026/05/16/arduino-ble-car/)
